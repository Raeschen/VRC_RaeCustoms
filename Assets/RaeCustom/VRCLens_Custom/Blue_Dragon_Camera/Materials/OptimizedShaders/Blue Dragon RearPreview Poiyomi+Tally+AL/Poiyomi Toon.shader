Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/641ad0c8804136e42b5bb37882d7ae09"
{
	Properties
	{
...
float2 decalCenter = float4(0.1,0.525,1,1);
float theta = radians(180.0 + _Time.z * 0.0);
float cs = cos(theta);
float sn = sin(theta);
uv = float2((uv.x - decalCenter.x) * cs - (uv.y - decalCenter.y) * sn + decalCenter.x, (uv.x - decalCenter.x) * sn + (uv.y - decalCenter.y) * cs + decalCenter.y);
uv = remap(uv, float2(0, 0) - float4(0.1,0.1,0.1,0.145).xz / 2 + float4(0.1,0.525,1,1), float4(0.1,0.1,0.1,0.145).yw / 2 + float4(0.1,0.525,1,1), float2(0, 0), float2(1, 1));
...
float maskVolume = 30.0 != 0.0 ? floor(maskY * 30.0) / 30.0 : maskY;
float maskBand = 15.0 != 0.0 ? floor(maskX * 15.0) / 15.0 : maskX;
...
{
float volumeth = 0.5;
if (30.0 != 0.0) audioLinkMask.b = frac(maskY * 30.0) > volumeth ? 0.0 : audioLinkMask.b;
float bandwidth = 0.0 == 1 ? 0.85 / aluv.y : 0.85;
float bandth = 1.0 - bandwidth;
if (15.0 != 0.0) audioLinkMask.b = frac(maskX * 15.0 + bandth * 0.5) < bandth ? 0.0 : audioLinkMask.b;
}
...
if (1.0)
{
poiLight.finalLighting = min(poiLight.finalLighting, 1.0);
}
}
#endif
