Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/6a2eea1af3f464d46942d9cdcd4e9357"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, inout PoiMods poiMods, in PoiLight poiLight)
			{
				...
				Decal0.m_DecalBlendType = 8.0;
				Decal0.m_DecalRotation = 180.0;
				Decal0.m_DecalScale = float4(0.11,0.061875,1,0);
				Decal0.m_DecalSideOffset = float4(0,0,0,0);
				Decal0.m_DecalPosition = float4(0.0965,0.545,0,0);
				Decal0.m_DecalRotationSpeed = 0.0;
				Decal0.m_DecalEmissionStrength = 0.5;
			...
			void ApplyAudioLinkDecal(in PoiMesh poiMesh, inout PoiFragData poiFragData, in PoiMods poiMods)
			{
				...
				float2 decalCenter = float4(0.1,0.48,1,1);
				float theta = radians(180.0 + _Time.z * 0.0);
				float cs = cos(theta);
				float sn = sin(theta);
				uv = float2((uv.x - decalCenter.x) * cs - (uv.y - decalCenter.y) * sn + decalCenter.x, (uv.x - decalCenter.x) * sn + (uv.y - decalCenter.y) * cs + decalCenter.y);
				uv = remap(uv, float2(0, 0) - float4(0.1,0.1,0.05,0.05).xz / 2 + float4(0.1,0.48,1,1), float4(0.1,0.1,0.05,0.05).yw / 2 + float4(0.1,0.48,1,1), float2(0, 0), float2(1, 1));
